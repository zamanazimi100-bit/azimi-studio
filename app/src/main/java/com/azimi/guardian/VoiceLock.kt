...
object VoiceLock {
    ...
    fun hasEnrollment(context: Context): Boolean {
        ...
        return state.enrolled && state.sampleCount >= ENROLLMENT_SAMPLES
    }
    ...
    fun verify(
        ...
        if (!hasEnrollment(activity)) {
            onResult(
                Result(
                    false,
                    "Voice Lock is not enrolled. Enroll the owner voice factor first."
                )
            )
            return
        }
        ...
        capture(activity) { feature, error ->
            ...
            if (template == null) {
                onResult(
                    Result(
                        false,
                        "Voice Lock template is unavailable. Enrollment is required again."
                    )
                )
                return@capture
            }
            ...
            if (score >= VERIFICATION_THRESHOLD) {
                ...
                if (session.isNullOrBlank()) {
                    onResult(
                        Result(
                            false,
                            "The biometric owner factor session is missing. Verify biometrics again."
                        )
                    )
                    return@capture
                }
                ...
                onResult(
                    Result(
                        true,
                        "Voice Lock verified for the current owner-authentication session.",
                        score
                    )
                )
                ...
                onResult(
                    Result(
                        false,
                        "Voice verification did not meet the local similarity threshold. Try again using the enrolled voice and phrase.",
                        score
                    )
                )
            }
     ...
     */
    internal fun consumeVerifiedFactor(
        context: Context
    ): Boolean {
        ...
        val valid =
            state.verifiedForSession &&
                !session.isNullOrBlank() &&
                session == state.verifiedSessionId(context)
    ...
    private fun runEnrollment(
        ...
        if (index >= ENROLLMENT_SAMPLES) {
            ...
            onResult(
                Result(
                    true,
                    "Voice Lock enrollment completed with $ENROLLMENT_SAMPLES local samples. A fresh voice verification is still required."
                )
            )
        ...
        capture(activity) { feature, error ->
            ...
            if (feature == null) {
                ...
                onResult(
                    Result(
                        false,
                        error ?: "Voice enrollment capture failed."
                    )
                )
    ...
    private fun capture(
        activity: Activity,
        callback: (FloatArray?, String?) -> Unit
    ) {
            ...
            try {
                ...
                if (minBuffer <= 0) {
                    ...
                    post(activity) {
                        callback(
                            null,
                            "This device does not expose a usable microphone configuration."
                        )
                    }
                ...
                while (offset < samples.size) {
                    ...
                    if (read <= 0) {
                        ...
                        post(activity) {
                            callback(
                                null,
                                "Microphone capture returned no usable audio."
                            )
                        }
                ...
                if (feature == null) {
                    ...
                    post(activity) {
                        callback(
                            null,
                            "Voice sample was too quiet or too short. Please speak clearly and try again."
                        )
                    }
                ...
                post(activity) {
                    callback(
                        null,
                        "Microphone permission is required for Voice Lock."
                    )
                }
                ...
                post(activity) {
                    callback(
                        null,
                        "Voice capture failed safely: ${t.javaClass.simpleName}."
                    )
                }
    ...
    private fun decrypt(
        ...
        require(data.isNotEmpty()) {
            "Encrypted Voice Lock template is empty."
        }
        ...
        require(
            ivLength > 0 &&
                data.size > 1 + ivLength
        ) {
            "Invalid Voice Lock encrypted template."
        }
    ...
    private fun android.content.SharedPreferences.getLongOrNull(
        key: String
    ): Long? {
    ...
    private fun State.verifiedSessionId(
        context: Context
    ): String? =
        prefs(context)
            .getString(
    ...
    private fun currentFactorSession(
        context: Context
    ): String? =
        context.applicationContext
            .getSharedPreferences(
